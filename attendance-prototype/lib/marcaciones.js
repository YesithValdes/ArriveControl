 ...
 */
export async function registrarPaso({ esquema, empleadoId, sedeId, tsDispositivo = null, diferido = false }) {
  ...
  return conEmpresa(esquema, async (client) => {
    const emp = await client.query(
      `select id, activo from empleados where id = $1`, [empleadoId],
    )
    ...
    const ins = await client.query(
      `insert into marcaciones (empleado_id, tipo, ts, ts_dispositivo, sede_id, origen)
